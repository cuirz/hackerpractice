	...
	for _, path := range ls {
		...
		if cnt == 0 {
			if cnt < len(depth) {
				...
				depth = append(depth, len(file))
			}
		} else {
			if cnt < len(depth) {
				depth[cnt] = depth[cnt-1] + len(file) + 1
			} else {
				depth = append(depth, depth[cnt-1]+len(file)+1)
			}
		}
		if isFile(file) {
			result = max(result, depth[cnt])
		}
	}
	return result
}
...
func main() {
	println(lengthLongestPath("file1.txt\nfile2.txt\nlongfile.txt"))
}
