...
func busiestServers(k int, arrival []int, load []int) []int {
	...
	for i, t := range arrival {
		for len(busy) > 0 && busy[0].task <= t {
			heap.Push(&available, i+((busy[0].id-i)%k+k)%k)
			heap.Pop(&busy)
		}
		if available.Len() == 0 {
			continue
		...
		request[id]++
		if request[id] > maxRequest {
			maxRequest = request[id]
			result = []int{id}
		} else if maxRequest == request[id] {
			result = append(result, id)

		}
		heap.Push(&busy, pair{id, t + load[i]})
	}
	return result
}
